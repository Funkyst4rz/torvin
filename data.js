...
const CLERIC_SPELLS = {
  ...
  1: [
    { id:'bane',        name:'Fléau',            tag:'DD Cha',   conc:true,  desc:"3 créatures subissent −1d4 à leurs jets d'attaque et de sauvegarde pendant 1 minute.", upcast:"+1 créature par niveau d'emplacement supérieur au niv.1." },
    { id:'bless',       name:'Bénédiction',       tag:'buff',     conc:true,  desc:"3 créatures ajoutent 1d4 à leurs jets d'attaque et de sauvegarde pendant 1 minute.", upcast:"+1 créature par niveau d'emplacement supérieur au niv.1." },
    { id:'command',     name:'Commandement',      tag:'contrôle', conc:false, desc:"Une créature obéit à un commandement d'un mot. JS Sagesse pour résister.", upcast:"+1 créature par niveau d'emplacement supérieur au niv.1." },
    { id:'curewounds',  name:'Soin des blessures',tag:'soin',     conc:false, desc:"Au toucher : la cible récupère 1d8 + mod Sag PV.", upcast:"+1d8 PV par niveau d'emplacement supérieur au niv.1." },
    { id:'guidingbolt', name:'Trait de lumière',  tag:'+atk/4d6', conc:false, desc:"Attaque sort à distance : 4d6 dégâts radiants. Prochaine attaque contre la cible avec avantage.", upcast:"+1d6 dégâts par niveau d'emplacement supérieur au niv.1." },
    { id:'healingword', name:'Mot de guérison',   tag:'soin bon.',conc:false, desc:"Action bonus : une créature récupère 1d4 + mod Sag PV.", upcast:"+1d4 PV par niveau d'emplacement supérieur au niv.1." },
    { id:'infwounds',   name:'Infliger des blessures',tag:'+atk/3d10',conc:false,desc:"Attaque sort au contact : 3d10 dégâts nécrotiques.", upcast:"+1d10 dégâts par niveau d'emplacement supérieur au niv.1." },
    ...
    { id:'detectevil',    name:'Détection du mal et du bien', tag:'divination', conc:true,  desc:"Détectez les aberrations, célestes, élémentaires, fées, fiélons et morts-vivants dans 9 m. Sentez les lieux/objets consacrés ou profanés." },
    { id:'detectmagic',   name:'Détection de la magie',       tag:'rituel',     conc:true,  desc:"Détectez la magie dans 9 m. Vous percevez son aura et son école. Durée 10 minutes." },
    { id:'detectpoison',  name:'Détect. poison et maladies',  tag:'rituel',     conc:true,  desc:"Détectez les poisons, créatures venimeuses et maladies dans 9 m pendant 10 minutes." },
    { id:'purifyfood',    name:"Purification nourriture/eau", tag:'rituel',     conc:false, desc:"Purifiez toute nourriture et eau non magique dans une sphère de 1,5 m de rayon, supprimant poisons et maladies." },
  ],
  2: [
    { id:'aid',         name:'Aide',              tag:'+5 PV',     conc:false, desc:"3 alliés gagnent +5 PV max et actuels pendant 8 heures.", upcast:"+5 PV supplémentaires par niveau d'emplacement supérieur au niv.2." },
    { id:'blinddeaf',   name:'Cécité / Surdité',  tag:'DD Con',    conc:false, desc:"JS Constitution ou aveugle ou sourd pendant 1 minute.", upcast:"+1 créature par niveau d'emplacement supérieur au niv.2." },
    { id:'holdperson',  name:'Immobilisation de personne',tag:'DD Sag',conc:true,desc:"JS Sagesse ou paralysée. Renouvellement possible chaque round.", upcast:"+1 créature par niveau d'emplacement supérieur au niv.2." },
    ...
    { id:'prayerheal',  name:'Prière de guérison', tag:'soin 10min',conc:false, desc:"6 créatures récupèrent 2d8 + mod Sag PV. Incantation de 10 minutes.", upcast:"+1d8 PV par niveau d'emplacement supérieur au niv.2." },
    ...
    { id:'spweapon',    name:'Arme spirituelle',   tag:'bon./1d8',  conc:false, desc:"Action bonus : arme spectrale qui attaque à chaque tour. Bonus prof + mod Sag.", upcast:"+1d8 dégâts par tranche de 2 niveaux au-dessus du niv.2 (slot niv.4 = 2d8, niv.6 = 3d8…)." },
    ...
    { id:'calmeemotions', name:'Apaisement des émotions',     tag:'contrôle',   conc:true,  desc:"JS Charisme ou les créatures dans 4,5 m sont neutralisées ou immunisées à la peur/charme pendant 1 minute." },
    { id:'contflame',     name:'Flamme continue',             tag:'utilitaire', conc:false, desc:"Flamme permanente (non soufflable par le vent) sur un objet. Produit autant de lumière qu'une torche." },
    { id:'enhanceability',name:'Amélioration de caract.',     tag:'buff',       conc:true,  desc:"Accordez l'un des effets suivants : Grâce du Taureau (+1d4 jets For, avantage For), Grâce du Chat (avantage Dex, pas de dégâts de chute < 6 m), Résistance de l'Ours (+2d6 PV temporaires), Bravoure du Renard (avantage Cha), Sérénité du Hibou (avantage Sag ou Int), Ardeur de l'Aigle (avantage Dex ou Str).", upcast:"+1 cible par niveau d'emplacement supérieur au niv.2." },
    { id:'findtraps',     name:'Détection des pièges',        tag:'divination', conc:false, desc:"Détectez la présence de pièges (mais pas leur emplacement précis) dans votre champ de vision." },
    { id:'gentlerepose',  name:'Repos éternel',               tag:'rituel',     conc:false, desc:"Préservez un cadavre de la décomposition et empêchez l'animation en mort-vivant pendant 10 jours." },
    { id:'locateobject',  name:"Localisation d'objet",        tag:'divination', conc:true,  desc:"Sentez la direction d'un objet familier ou d'un type d'objet dans 300 m pendant 10 minutes." },
    { id:'zoneoftruth',   name:'Zone de vérité',              tag:'utilitaire', conc:false, desc:"Sphère 4,5 m : JS Charisme ou impossible de mentir sciemment pendant 10 minutes." },
  ],
  3: [
    { id:'animdead',    name:'Animation des morts',tag:'nécro',     conc:false, desc:"Animez un squelette ou zombie obéissant pendant 24h. Renouvelable.", upcast:"+2 morts-vivants supplémentaires par niveau d'emplacement supérieur au niv.3." },
    ...
    { id:'bestowcurse', name:'Malédiction',         tag:'debuff',    conc:true,  desc:"Désavantage sur jets d'une caractéristique, −1d8 aux dégâts ou perte de tour.", upcast:"Niv.4 : 10 min sans concentration. Niv.5 : 8 heures. Niv.7 : 24 heures. Niv.9 : jusqu'à dissipation." },
    ...
    { id:'massHW',      name:'Soins de groupe',     tag:'soin bon.', conc:false, desc:"Action bonus : jusqu'à 6 créatures récupèrent 1d4 + mod Sag PV.", upcast:"+1d4 PV par niveau d'emplacement supérieur au niv.3." },
    ...
    { id:'spiritguard', name:'Esprits gardiens',    tag:'zone',      conc:true,  desc:"Spectres 4,5 m infligent 3d8 dégâts radiants ou nécrotiques aux ennemis.", upcast:"+1d8 dégâts par niveau d'emplacement supérieur au niv.3." },
    ...
    { id:'clairvoyance',  name:'Clairvoyance',               tag:'divination', conc:true,  desc:"Créez un œil ou une oreille invisible dans un lieu connu à 1,6 km. Perception via ce sens pendant 10 minutes." },
    { id:'createfoodwater',name:'Création de nourriture/eau',tag:'utilitaire', conc:false, desc:"Créez 25 kg de nourriture et 100 litres d'eau potable — suffit pour 15 humanoïdes et 5 montures 24h." },
    { id:'dispelmagic',   name:'Dissipation de la magie',    tag:'utilitaire', conc:false, desc:"Mettez fin à tous les sorts sur une cible (niv. ≤ 3 auto, niv. supérieur : JS Archane).", upcast:"Dissipe automatiquement les sorts de niveau égal au slot utilisé." },
    { id:'feigndeath',    name:'Feindre la mort',            tag:'rituel',     conc:false, desc:"Au toucher : la cible entre dans un état de mort apparente pendant 1 heure (ou jusqu'à réveil). Résistance à tous dégâts sauf psychiques." },
    { id:'glyphwarding',  name:'Glyphe de protection',       tag:'piège',      conc:false, desc:"Inscrivez un glyphe qui déclenche un sort ou une explosion d'énergie (5d8) quand une condition est remplie." },
    { id:'magiccircle',   name:'Cercle magique',             tag:'défense',    conc:false, desc:"Cercle 3 m : bloque les aberrations, célestes, élémentaires, fées ou fiélons (votre choix) pendant 1h.", upcast:"+1 heure par niveau d'emplacement supérieur au niv.3." },
    { id:'meldstone',     name:'Fusion avec la pierre',      tag:'rituel',     conc:false, desc:"Fusionnez avec une pierre ou roche adjacente et devenez indétectable. Durée 8 heures." },
    { id:'speakdead',     name:'Parler aux morts',           tag:'divination', conc:false, desc:"Un cadavre répond à 5 questions. Il répond selon ses connaissances et sa personnalité passée." },
    { id:'waterwalk',     name:'Marcher sur l\'eau',         tag:'rituel',     conc:false, desc:"Jusqu'à 10 créatures se déplacent sur les liquides pendant 1 heure." },
  ],
  4: [
    { id:'banishment',  name:'Bannissement',        tag:'contrôle',  conc:true,  desc:"JS Charisme ou banni pendant 1 min. Si extra-planaire, permanent après 1 minute.", upcast:"+1 créature par niveau d'emplacement supérieur au niv.4." },
    ...
    { id:'controlwater',  name:"Contrôle de l'eau",           tag:'contrôle',   conc:true,  desc:"Inondation, tourbillon (3d8), séparation des eaux ou redirection du courant sur 30 m³ d'eau." },
    { id:'locatecreature',name:'Localisation de créature',    tag:'divination', conc:true,  desc:"Sentez la direction d'une créature familière ou d'une espèce dans 300 m pendant 1 heure." },
    { id:'stoneshape',    name:'Façonnage de la pierre',      tag:'utilitaire', conc:false, desc:"Façonnez une pierre touchée (max 1,5 m³) selon votre volonté." },
  ],
  5: [
    ...
    { id:'flamestrike', name:'Colonne de flamme',    tag:'+atk',     conc:false, desc:"Cylindre 3 m × 9 m : 4d6 feu + 4d6 radiants. JS Dex pour moitié.", upcast:"+1d6 feu + 1d6 radiant par niveau d'emplacement supérieur au niv.5." },
    ...
    { id:'masscure',    name:'Soins de groupe supérieurs',tag:'soin',conc:false, desc:"Jusqu'à 6 créatures dans 9 m récupèrent 3d8 + mod Sag PV.", upcast:"+1d8 PV par niveau d'emplacement supérieur au niv.5." },
    ...
    { id:'contagion',     name:'Contagion',                   tag:'debuff',     conc:false, desc:"Au toucher (jet d'attaque) : infligez une maladie (Fièvre aveuglante, Pourriture, Fièvre putride, Convulsions, Esprit enfiévré ou Pourriture ventre)." },
    { id:'geas',          name:'Geis',                        tag:'contrôle',   conc:false, desc:"JS Sagesse ou la cible obéit à une instruction monosyllabique pendant 30 jours (7d10 psychiques si désobéit).", upcast:"Niv.7 : durée 1 an. Niv.9 : permanent." },
    { id:'hallow',        name:'Sanctification',              tag:'zone',       conc:false, desc:"Consacrez un lieu (18 m de rayon). Bloquez les créatures d'un type, accordez résistance, immunité à peur/charme, etc. (1 000 po, incantation 24h)." },
    { id:'insectplague',  name:"Nuée d'insectes",             tag:'zone/DD Con',conc:true,  desc:"Sphère 6 m d'insectes voraces : 4d10 piqûres (JS Con pour moitié). Terrain difficile, vue bloquée.", upcast:"+1d10 dégâts par niveau d'emplacement supérieur au niv.5." },
    { id:'planarbinding', name:'Liens planaires',             tag:'contrôle',   conc:false, desc:"Liez une aberration, céleste, élémentaire ou fiélon (JS Cha ou vous sert 24h).", upcast:"Durée augmente : niv.6 = 10 jours, niv.7 = 30 jours, niv.8 = 180 jours, niv.9 = 1 an." },
  ],
};
